...
const source = readFileSync(resolve(__dirname, 'ChatPieChart.svelte'), 'utf-8')

describe('ChatPieChart', () => {
  it('compiles without errors', () => {
    const result = compile(source, {
      filename: 'ChatPieChart.svelte',
      generate: 'client',
    })
  ...
  it('calls Plotly.newPlot with pie trace type', () => {
    const result = compile(source, {
      filename: 'ChatPieChart.svelte',
      generate: 'client',
    })
    expect(result.js.code).toMatch(/newPlot/)
    expect(result.js.code).toMatch(/type.*pie/)
  })
