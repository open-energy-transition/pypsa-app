...
const source = readFileSync(resolve(__dirname, 'ChatBarChart.svelte'), 'utf-8');

describe('ChatBarChart', () => {
  it('compiles without errors', () => {
    const result = compile(source, {
      filename: 'ChatBarChart.svelte',
      generate: 'client',
    });
  ...
  it('calls Plotly.newPlot with bar trace type', () => {
    const result = compile(source, {
      filename: 'ChatBarChart.svelte',
      generate: 'client',
    });
    expect(result.js.code).toMatch(/newPlot/);
    expect(result.js.code).toMatch(/type.*bar/);
  });
